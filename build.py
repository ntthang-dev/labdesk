...
def build_flutter_dmg(version, features):
    ...
    system2(
        f'FLUTTER_XCODE_ARCHS={mac_arch} FLUTTER_XCODE_ONLY_ACTIVE_ARCH=YES flutter build macos --release {flutter_args}'.strip())
    # PRODUCT_NAME is configurable in AppInfo.xcconfig, so glob instead of hardcoding it.
    system2('cp -rf ../target/release/service ./build/macos/Build/Products/Release/*.app/Contents/MacOS/')
    '''
    system2(
...
def build_flutter_windows(version, features, skip_portable_pack):
    ...
    os.chdir('flutter')
    flutter_args = os.environ.get('FLUTTER_BUILD_ARGS', '')
    system2(f'flutter build windows --release {flutter_args}'.strip())
    os.chdir('..')
